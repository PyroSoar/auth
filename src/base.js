  ...
  async getUserInfo() {
    ...
    if (finalRedirect && finalRedirect.startsWith('http') && this.ctx.headers['user-agent'] !== '@waline') {
      try {
        ...
        // Append platform type if present in state
        const parsedState = qs.parse(this.ctx.query?.state || this.ctx.params?.state || '');
        if (parsedState.type) {
          url.searchParams.set('type', parsedState.type);
          console.log('[Base.getUserInfo] Appended platform type:', parsedState.type);
        }
        console.log('[Base.getUserInfo] Browser phase: Redirecting to client:', url.toString());
    ...
    // Step 5: Server phase → return JSON user info
    this.ctx.type = 'json';
    try {
      console.log('[Base.getUserInfo] Server phase: Fetching access token...');
      ...
      console.error('[Base.getUserInfo] Error in getUserInfo:', error.message);
      this.ctx.status = 500;
